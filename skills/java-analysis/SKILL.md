...
- **完整向上调用链**（追到 REST、多起点合并等）请用 **`callchain`** 子命令或 skill `java-callchain-analysis`，勿与本文混用。
...
- **`incomingCalls` / `outgoingCalls`**：仅取 `prepareCallHierarchy` 的**第一个**锚点的一层调用；多候选时需移动光标或换用 `callchain`。
