...
| 目标 | 说明 |
|------|------|
| **正确性** | `analyze` 各 operation 与 LiteClaw `lsp_java_analyze` 语义一致；`callchain` 入口解析、向上追踪、终止原因与输出 schema 符合 README |
...
- **ripgrep (`rg`)**：关键字 `callchain` 走 `java_text_grep` 时与纯 Python 扫描路径应对照各测一轮
...
| U3 | **关键字拆分** | `callchain`：`query` 中 `\|` / `｜` 拆分、单段 vs 多段行为与 README 一致 |
| U4 | **CLI 参数互斥** | `callchain`：类+方法 / 文件+行 / query **三选一**；缺参、多选报错 |
...
- 连续运行多条命令：**JDTLS 子进程退出**、无僵尸（可用脚本跑 N 次 `analyze` + 一次 `callchain`）。
...
| 类型 | 做法 |
|------|------|
| **黄金输出** | 对 fixture 项目固定 `analyze`/`callchain` 的 **JSON 快照**（路径脱敏）；变更 JDTLS 版本时重审 |
...
3. `tests/integration/`：analyze + callchain，标记 `@pytest.mark.integration`。  
...
- **索引时序**：`workspaceSymbol` / callchain 首次可能空结果，与 liteclaw 类似可考虑 **重试**（集成测试 sleep 或轮询）。  
